...
def mantle_base_url(region: str) -> str:
    env_url = os.environ.get("MANTLE_GEMMA_BASE_URL") or os.environ.get("BEDROCK_MANTLE_URL")
    if env_url:
        return mantle_base_url_from_value(env_url)
    return f"https://bedrock-mantle.{region}.api.aws/openai/v1"


def mantle_base_url_from_value(base_url: str) -> str:
    trimmed = base_url.rstrip("/")
    # Mantle lists models on /v1, but Gemma chat-completions are served
    # through the OpenAI-compatible /openai/v1 route for this account.
    if trimmed.endswith("/v1") and not trimmed.endswith("/openai/v1"):
        return trimmed[: -len("/v1")] + "/openai/v1"
    return trimmed


def mantle_model_list_base_url(generation_base_url: str) -> str:
    trimmed = generation_base_url.rstrip("/")
    if trimmed.endswith("/openai/v1"):
        return trimmed[: -len("/openai/v1")] + "/v1"
    return trimmed
...
def accuracy_prompt(item: AccuracyItem) -> str:
    if item.answer_type == "choice":
        ...
        return (
            "Answer the multiple-choice question. Return only the letter A, B, C, or D.\n\n"
...
def run_benchmark(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    base_url = mantle_base_url(args.region) if not args.base_url else mantle_base_url_from_value(args.base_url)
    ...
    if args.list_models:
        try:
            model_list_base_url = mantle_model_list_base_url(base_url)
            model_list_client = load_openai_client(
                api_key=api_key,
                base_url=model_list_base_url,
                timeout=args.timeout,
                max_retries=args.max_retries,
            )
